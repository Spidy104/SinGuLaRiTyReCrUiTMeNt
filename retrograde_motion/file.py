...
def plot_retrograde_motion():
    ...
    for i, (planet, color) in enumerate(zip(planets, colors)):
        ...
        # Plot RA vs time
        ax1 = axes[i, 0]
        ax1.plot(times.datetime, ra, color=color, linewidth=2, alpha=0.7, label=f'{planet.title()} RA')
        
        # Highlight retrograde periods
        retrograde_times = times[1:][retrograde_mask]
        retrograde_ra = ra[1:][retrograde_mask]
        ax1.scatter(retrograde_times.datetime, retrograde_ra, 
                   color='darkred', s=20, alpha=0.8, zorder=5, label='Retrograde Motion')
        ...
        ax2 = axes[i, 1]
        ax2.plot(ra, dec, color=color, linewidth=2, alpha=0.7, label=f'{planet.title()} Path')
        ...
        ax2.plot(retrograde_ra_path, retrograde_dec_path, 
                color='darkred', linewidth=3, alpha=0.9, label='Retrograde Loops')
        
        # Mark start and end points
        ax2.scatter(ra[0], dec[0], color='green', s=100, marker='o', 
                   zorder=5, label='Start', edgecolor='black')
        ax2.scatter(ra[-1], dec[-1], color='blue', s=100, marker='s', 
                   zorder=5, label='End', edgecolor='black')
        ...
        print(f"{planet.title()} retrograde motion: {total_retrograde_days:.1f} days total")
...
def plot_mars_detailed_retrograde():
    ...
    fig.suptitle('Mars Retrograde Motion - Detailed View (Aug 2022 - Feb 2023)', 
    ...
    retrograde_times = times[1:][retrograde_mask]
    retrograde_ra = ra[1:][retrograde_mask]
    ax1.plot(retrograde_times.datetime, retrograde_ra, 'darkred', 
             linewidth=3, alpha=0.9, label='Retrograde Period')
    ...
    retrograde_dec_path = dec[1:][retrograde_mask]
    ax2.plot(retrograde_ra_path, retrograde_dec_path, 
             'darkred', linewidth=4, alpha=0.9, label='Retrograde Loop')
    
    # Add arrows to show direction
    for j in range(0, len(ra)-10, 20):
        dx = ra[j+5] - ra[j]
        dy = dec[j+5] - dec[j]
        ax2.arrow(ra[j], dec[j], dx*0.3, dy*0.3, 
...
if __name__ == "__main__":
    ...
    print("- The loops in the sky path show the characteristic retrograde motion pattern")